...
import { readExcelSheet } from "@/lib/excel.util";
...
const [loanStatusData, setLoanStatusData] = React.useState<any[]>([]);
const [bankOffers, setBankOffers] = React.useState<any[]>([]);
const [loading, setLoading] = React.useState(true);
const [error, setError] = React.useState<string | null>(null);


React.useEffect(() => {
  async function loadNbfcExcel() {
    try {
      setLoading(true);

      const file = "/excels/Bank_Data.xlsx";
      console.log("Fetching Excel from:", "/data/Bank_Data.xlsx");


      const roi = await readExcelSheet(file, "ROI");
      const loanStatus = await readExcelSheet(file, "LoanStatus");
      const offers = await readExcelSheet(file, "BankOffers");
                    ...
                    <BarChart data={loanStatusData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={32}>
                        {loanStatusData.map((entry, index) => (
                          <Cell
                              key={`cell-${index}`}
                              fill={loanStatusColors[entry.name] || "#94a3b8"}
                            />
                        ))}
                      </Bar>
                    </BarChart>
